...
# --- helper: mesh → thumbnail + X-extent ---
...
# --- 1) equal-count edges for 1D → global bins ---
...
organelle_df["x_bin"] = pd.cut(x, bins=x_edges,
                               labels=False, include_lowest=True)
organelle_df["y_bin"] = pd.cut(y, bins=y_edges,
                               labels=False, include_lowest=True)

# --- 2) sample one per cell (if any)  ---
sampled = (
...
# --- 3) render thumbnails & record extents ---
thumbs = {}
x_extents = []
...
# --- 4) plot on a unit-grid with directional labels ###
fig, ax = plt.subplots(figsize=(12,12), dpi=150)

# treat each bin as a 1×1 square, so centers at (i+0.5, j+0.5)
filled = set(zip(sampled.x_bin.astype(int),
                 sampled.y_bin.astype(int)))
...
# directional labels
ax.text(grid_size/2, -0.3,
        "lsp (nm) increases \u2192",
        ha="center", va="center", fontsize=14)
ax.text(-0.3, grid_size/2,
        "Approximate Sphere Fraction increases \u2192",
